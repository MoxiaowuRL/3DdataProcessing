...
from pytorch3d.renderer import (
    FoVPerspectiveCameras, look_at_rotation, look_at_view_transform,
...
# the purpose for phong render is to have sharp images for comparison with soft render
raster_settings =RasterizationSettings(
    image_size=256,
    blur_radius=0.0,
    faces_per_pixel=1,  
...
# define carmera location and corresponding camera rotation and position
distance = 3
elevation = 50
azimuth = 0
...
class Model(nn.Module):
    def __init__(self, meshes, renderer_silhouette, renderer_textured, image_ref, weight_silhouette, 
                 weight_texture):
        super().__init__()
        self.meshes = meshes
        ...
        self.camera_position = nn.Parameter(torch.from_numpy(np.array([3, 6.9, +2.5], dtype = np.float32)).to(meshes.device))

    def forward(self):
        #Render the image using the updated camera position. Based on the new position of the camera
        #we calculate the rotation and translation matrices
...
# create a model using the renderer, mesh and reference image
...
for i in range(0, 200):
    ...
    optimizer.step()
    plt.figure()
    plt.imshow(image_silhouette.detach().squeeze().cpu().numpy()[..., 3])
    plt.title("iter: %d, loss: %0.2f" % (i, loss))
    plt.axis('off')
    plt.savefig(os.path.join(output_dir, 'soft_silhouette_%03d.png' % i))
    plt.close()
    ...
    plt.title("iter: %d, loss: %0.2f" % (i, loss))
    plt.axis('off')
    plt.savefig(os.path.join(output_dir, 'soft_texture_%03d.png' % i))
    plt.close()
    ...
    plt.title("iter: %d, loss: %0.2f" % (i, loss))
    plt.axis('off')
    plt.savefig(os.path.join(output_dir, 'hard_silhouette_%03d.png' % i))
    plt.close()
    ...
    plt.title("iter: %d, loss: %0.2f" % (i, loss))  
    plt.axis('off')
    plt.savefig(os.path.join(output_dir, 'hard_texture_%03d.png' % i))  
    plt.close()
