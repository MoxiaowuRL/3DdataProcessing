...
class VolumeModel(torch.nn.Module):
    def __init__(self, renderer, volume_size = [64] * 3, voxel_size = 0.1):
        ...
        self.log_densities = torch.nn.Parameter(-4 * torch.ones(1, *volume_size))
...
# huber function is a robust loss function to prevent a small number of outliers from dragging the optimization 
# away from the true optimal solution
...
for i in range(n_iter):
    if i == round(n_iter * 0.75):
        ...
        optimizer = torch.optim.Adam(volume_model.parameters(), lr = lr / 10)
...
def generate_rotating_volume(volume_model, n_frames = 50):
    ...
    logRs[:, 1] = torch.linspace(0, 2 * math.pi, n_frames, device = device)
    ...
    for R, T in zip(Rs, Ts):
        ...
        frames.append(volume_model(camera)[...,:3].clamp(0, 1))
