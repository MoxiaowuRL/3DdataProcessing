...
# monte carlo raysampler for training model
...
# NDCMultionomialRaysampler for rendering model 
render_size = target_images.shape[1] * 2
volume_extent_world = 3
...
def sample_images_at_mc_locs(target_images, sampled_rays_xy):
    ...
    # grid_sample function uses interpolation for accurate pixel values while NDC(normalized device coordinates) grid raysampler maps NDC coordinates to pixel coordinates
    images_sampled = torch.nn.functional.grid_sample(
        target_images.permute(0, 3, 1, 2),
        -sampled_rays_xy.view(ba, 1, -1, 2),   # sign inversed here
        align_corners = True, 
    )
...
for i in range(n_iter):
    if i == round(n_iter * 0.75):
        print('Reducing learning rate 10-fold')
    ...
    fig.savefig(f'./result_nerf/intermediate_{i:05d}.png')

# take final resulting volumetric model and render images from new angles
with torch.no_grad():
